...
class mailBase:
    def __init__(self, **args):
        ...
        try:
            ...
            print('error', '%s connect error' % self.host, 2)
    ...
    def sendMail(self, subject, content, subtype='plain', charset='utf-8', *attachs):
        ...
        for attach in attachs:
            ...
            if not os.path.isfile(attach):
                print('error', '%s not exists' % attach, 2)
...
if __name__ == "__main__":
    ...
    mail_dict['mail_send'] = "whaley_alert@whaley.cn"  # send
    mail_dict['mail_rece']='wang.baozhi@whaley.cn,lian.ka@whaley.cn'   #mail to
    m = mailBase(**mail_dict)
    sub = "监控实时处理延迟"
    content = "延迟topic为"+sys.argv[1]+"，延迟时间为"+sys.argv[2]+"s"
    #attachs = '/root/miles/script/yunwei/commons/mftp.py,/root/miles/script/yunwei/commons/mmysql.py'.split(',')
    attachs = []
