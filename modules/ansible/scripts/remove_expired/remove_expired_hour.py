...
def initlog():
    ...
    hdlr = logging.FileHandler("/data/scripts/default_hour.log")
...
def compare_file_time(file, houses):
    import time
    time_of_last_access = os.path.getctime(file)
    age_in_houses = (time.time() - time_of_last_access) / (60 * 60)
    if age_in_houses > int(houses):
        return True
    return False


def main():
    configName = "/data/scripts/remove_expired_hour.ini"
    data = getConfig(configName)
    for _data in data:
        path = _data.get("var_path", None)
        expired = _data.get("expired", 120)
