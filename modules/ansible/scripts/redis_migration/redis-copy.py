...
class RedisCopy:
    ...
    def __init__(self, source, target, dbs, d_dbs):
        self.source = source
        self.target = target
        self.dbs = dbs
        self.d_dbs = d_dbs
    ...
    def save_keylists(self, prefix="*"):
        ...
        for db in self.dbs:
            ...
            r = redis_get_values.StrictRedis(
    ...
    def copy_db(self, limit=None):
        ...
        for i in range(len(self.dbs)):
            servername = self.source['host'] + ":" + str(
                self.source['port']) + ":" + str(self.dbs[i])
            ...
            r = redis_get_values.StrictRedis(
                host=self.source['host'], port=self.source['port'], db=self.dbs[i])
            moved = 0
            dbsize = r.dbsize() - 1
            ...
            rr = redis_get_values.StrictRedis(
                host=self.target['host'], port=self.target['port'], db=self.d_dbs[i])
    ...
    def flush_target(self):
        ...
        for i in range(len(self.dbs)):
            servername = self.target['host'] + ":" + str(
                self.target['port']) + ":" + str(self.dbs[i])
...
def main(source, target, databases, destdb=None, limit=None, clean=False, flush=False, prefix="*"):
    ...
    if not destdb:
        ...
        d_dbs = [int(o) for o in destdb.split(',')]
    if len(d_dbs) < 1:
        exit('Supplied list of db is wrong. e.g. python redis-copy.py 127.0.0.1:6379 127.0.0.1:63791  0,1  0,1')
    try:
        r = redis_get_values.StrictRedis(
            host=source_server['host'], port=source_server['port'], db=dbs[0])
    except AttributeError as e:
        exit('Please this script requires redis-py >= 2.4.10, your current version is :' + redis_get_values.__version__)

    mig = RedisCopy(source_server, target_server, dbs, d_dbs)
...
if __name__ == "__main__":
    ...
    prefix = "*"
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hl:s:t:d:m:fp:",
                                   ["help", "limit=", "source=", "target=", "databases=", "destdb=", "clean", "flush",
                                    "prefix="])
...
try:
    main(source, target, databases, destdb, limit, clean, flush, prefix)
